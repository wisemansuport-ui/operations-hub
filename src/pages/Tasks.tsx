...
const MetaInterior = ({ meta, onBack, onUpdateMeta, addNotification, users, activeOperator, role }: { meta: OperationMeta, onBack: () => void, onUpdateMeta: (m: OperationMeta) => void, addNotification: (n: any) => void, users: any[], activeOperator: string, role: 'ADMIN' | 'OPERADOR' }) => {
  ...
  const totalAutoSalarios = remessas.reduce((acc, r) => {
    if (meta.modelo === 'Recarga' || r.naoContabilizarSalario) return acc;
  ...
  const handleFinishMeta = () => {
    ...
    setTimeout(() => {
      ...
      pushNotify(
        ...
        `Meta finalizada com ${totalContas} contas | L/P: ${resultadoLiquido >= 0 ? '+' : ''}R$ ${resultadoLiquido.toFixed(2)}`,
        targetAdmins
      );
        ...
        <form onSubmit={onRegister} className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {!meta.isAdminMeta ? (
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase flex items-center gap-1.5">Depósitos na Média {!isRecarga && <span className="text-primary">(R$ 2)</span>}</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase flex items-center gap-1.5">Depósitos Baixos {!isRecarga && <span className="text-primary">(R$ 1)</span>}</label>
      ...
      {displayList.length === 0 ? (
        ...
        <div data-tour="tasks-meta-entry" className="mt-4 space-y-3">
          {paginatedList.map(meta => {
            ...
            return (
              <div key={meta.id} className={meta.isAdminMeta ? "glass-card bg-[#0a0a0a]/95 rounded-2xl border border-primary/20 p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:border-primary/50 hover:bg-[#0f0f0f] hover:shadow-[0_8px_30px_-10px_hsl(var(--primary)/0.25)] hover:-translate-y-0.5 transition-all relative overflow-hidden group cursor-pointer" : "glass-card rounded-2xl border border-border/30 p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:border-primary/40 hover:shadow-[0_8px_30px_-10px_hsl(var(--primary)/0.15)] hover:-translate-y-0.5 transition-all relative overflow-hidden group cursor-pointer"} onClick={() => setSelectedMetaId(meta.id)}>
                <div className={`absolute left-0 top-0 bottom-0 w-1 transition-colors ${meta.status === 'fechada' ? 'bg-primary/40' : meta.status === 'lixeira' ? 'bg-red-500' : meta.isAdminMeta ? 'bg-gradient-to-b from-primary via-primary/80 to-transparent shadow-[0_0_15px_hsl(var(--primary)/0.6)]' : 'bg-gradient-to-b from-primary via-primary/80 to-transparent shadow-[0_0_15px_hsl(var(--primary)/0.5)]'}`} />
                <div className="flex items-center gap-4 min-w-0 flex-1">
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 shadow-[0_0_8px_currentColor] transition-colors ${meta.status === 'fechada' ? 'text-primary/40 bg-primary/40' : meta.status === 'lixeira' ? 'text-red-500 bg-red-500' : meta.isAdminMeta ? 'text-primary bg-primary animate-[pulse_2s_ease-in-out_infinite]' : 'text-primary bg-primary animate-[pulse_2s_ease-in-out_infinite]'}`} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      {meta.isAdminMeta && <span className="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest bg-primary/15 text-primary border border-primary/40 mr-1 shadow-[0_0_10px_hsl(var(--primary)/0.25)]">Admin</span>}
