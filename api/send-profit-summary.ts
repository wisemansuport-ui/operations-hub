  ...
  try {
    do {
      ...
      if (!res.ok) {
        ...
        if (opts.optional) {
          console.warn(`[fetchCollection:${name}] ${res.status} — degradando.`, txt.slice(0, 200));
          return docs;
        }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    for (const [admin, total] of Array.from(profitByAdmin.entries())) {
